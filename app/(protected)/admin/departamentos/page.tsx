...
export default function AdminDepartamentosPage() {
  ...
  const fetchDepartamentos = useCallback(async () => {
    ...
    try {
      ...
      if (json.ok) setDepartamentos(json.data);
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editandoId) {
        const res = await fetch(`/api/admin/departamentos/${editandoId}`, {
          method: "PUT",
  ...
  const toggleActivo = async (d: Departamento) => {
    setError(""); setSuccessMsg("");
    try {
      const res = await fetch(`/api/admin/departamentos/${d.id}`, {
        method: "DELETE",
      });
      const json = await res.json();
      if (!json.ok) { setError(json.message); return; }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#64748B] mt-1">
            {departamentos.length} departamentos registrados
          </p>
        </div>
        <button
          onClick={abrirCrear}
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                departamentos.map((d) => (
                  <tr key={d.id} className="border-b border-[#F1F5F9] hover:bg-[#FAFBFC] transition-colors">
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-[#EFF6FF] to-[#DBEAFE] flex items-center justify-center shrink-0">
                          <Building2 size={15} className="text-[#2563EB]" strokeWidth={2} />
                        </div>
                        <div>
                          <span className="text-[13px] font-semibold text-[#0F172A]">{d.nombre}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center justify-center gap-0.5">
                        <button onClick={() => abrirEditar(d)} className="p-1.5 rounded-lg hover:bg-[#EFF6FF] text-[#94A3B8] hover:text-[#2563EB] transition-colors" title="Editar departamento">
                          <Pencil size={14} strokeWidth={1.5} />
                        </button>
                        <button onClick={() => toggleActivo(d)} className={`p-1.5 rounded-lg transition-colors ${
                          d.activo
                          ...
                          {d.activo ? <Trash2 size={14} strokeWidth={1.5} /> : <RotateCcw size={14} strokeWidth={1.5} />}
                        </button>
                      </div>
                    </td>
                  </tr>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: DEPARTAMENTO DE TRÁNSITO"
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-[#E2E8F0]">
                ...
                <button type="submit" disabled={saving} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-[#2563EB] rounded-lg hover:bg-[#1D4ED8] disabled:bg-[#93C5FD] disabled:opacity-60 transition-colors">
                  ...
                  {editandoId ? "Guardar" : "Crear Departamento"}
                </button>
              </div>
            </form>
          </div>

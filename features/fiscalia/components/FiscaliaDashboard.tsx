...
const STATUS_TABS: { key: EstatusFiscalia; label: string; icon: typeof Clock; color: string; accent: string; bg: string }[] = [
    ...
    { key: 'EN_REVISION', label: 'En Revisión', icon: RefreshCw, color: '#3B82F6', accent: '#1E40AF', bg: '#EFF6FF' },
...
const STATUS_BADGE: Record<string, { bg: string; text: string; dot: string; label: string }> = {
    ...
    EN_REVISION: { bg: '#DBEAFE', text: '#1E40AF', dot: '#3B82F6', label: 'En Revisión' },
...
export default function FiscaliaDashboard({
    ...
    const estadisticas = useMemo(() => {
        ...
        const revision = data.filter(x => x.estatus_dependencia === 'EN_REVISION').length
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {STATUS_TABS.map(tab => {
                    const count = estadisticas[tab.key === 'PENDIENTE' ? 'pendientes' : tab.key === 'EN_REVISION' ? 'revision' : 'liberadas']

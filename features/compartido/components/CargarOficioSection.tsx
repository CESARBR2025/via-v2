...
export default function CargarOficioSection({
    ...
    const totalPasos = esTitularState === false ? 5 : 4
        ...
        <div>
            ...
            <div className="p-5 space-y-5">
                <>
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <div className="inline-flex items-center rounded-md p-0.5 bg-white border border-slate-200">
                                    <button type="button" onClick={() => { setEsTitularState(true); setPaso(3) }} className={`px-4 py-1.5 rounded text-[13px] font-medium transition-all duration-150 ${esTitularState === true ? 'bg-blue-700 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>Sí</button>
                                    <button type="button" onClick={() => { setEsTitularState(false); setPaso(3) }} className={`px-4 py-1.5 rounded text-[13px] font-medium transition-all duration-150 ${esTitularState === false ? 'bg-blue-700 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>No</button>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <button onClick={() => setPaso(1)} className="inline-flex items-center justify-center gap-2 rounded-lg py-2.5 px-4 text-[13px] font-medium text-slate-600 bg-transparent border border-slate-200 hover:bg-slate-50 hover:border-slate-300 active:bg-slate-100 transition-colors duration-150">
                    ...
                    {/* Paso 3: Documento (si Sí) o Datos titular (si No) */}
                    {paso === 3 && esTitularState === true && (
                        <div className="space-y-4">
                            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                                    <div className="p-2.5 rounded-lg bg-white border border-slate-200">
                                        ...
                                        <p className="text-xs font-medium text-slate-900 truncate">{[nombreInfractor, appaternoInfractor, apmaternoInfractor].filter(Boolean).join(' ') || '—'}</p>
                                    </div>
                                    <div className="p-2.5 rounded-lg bg-white border border-slate-200">
                                        <p className="text-[10px] font-medium tracking-wider uppercase text-slate-400 mb-0.5">Correo</p>
                                        <p className="text-xs font-medium text-slate-900 truncate">{correoInfractor || '—'}</p>
                                    </div>
                                    <div className="p-2.5 rounded-lg bg-white border border-slate-200">
                                        <p className="text-[10px] font-medium tracking-wider uppercase text-slate-400 mb-0.5">CURP</p>
                                        <p className="text-xs font-mono font-medium text-slate-900">{curpInfractor || '—'}</p>
                                    </div>
                                </div>
                            </div>
                            <FileUpload archivo={archivo} setArchivo={setArchivo} fileRef={fileRef} error={errores.archivo} setError={setErrores} />
                            <div className="flex items-center gap-3">
                                <button onClick={() => { setEsTitularState(null); setPaso(2) }} className="inline-flex items-center justify-center gap-2 rounded-lg py-2.5 px-4 text-[13px] font-medium text-slate-600 bg-transparent border border-slate-200 hover:bg-slate-50 hover:border-slate-300 active:bg-slate-100 transition-colors duration-150">
                                    <ArrowRight size={14} strokeWidth={2.5} className="rotate-180" /><span>Regresar</span>
                                </button>
                                <button onClick={() => { if (validarArchivo()) setPaso(4) }} className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-medium text-white bg-blue-700 hover:bg-blue-800 active:bg-blue-900 active:scale-[0.99] shadow-sm transition-all duration-150">
                    ...
                    {/* Paso 3 (No): Capturar datos del titular */}
                    {paso === 3 && esTitularState === false && (
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => { if (validarTitular()) setPaso(4) }} className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-medium text-white bg-blue-700 hover:bg-blue-800 active:bg-blue-900 active:scale-[0.99] shadow-sm transition-all duration-150">
                                    <span>Continuar</span><ArrowRight size={14} strokeWidth={2.5} />
                                </button>
                            </div>
                        </div>
                    ...
                    {/* Paso 4 (No): Documento */}
                    {paso === 4 && esTitularState === false && (
                        <div className="space-y-4">
                            <FileUpload archivo={archivo} setArchivo={setArchivo} fileRef={fileRef} error={errores.archivo} setError={setErrores} />
                            <div className="flex items-center gap-3">
                                <button onClick={() => setPaso(3)} className="inline-flex items-center justify-center gap-2 rounded-lg py-2.5 px-4 text-[13px] font-medium text-slate-600 bg-transparent border border-slate-200 hover:bg-slate-50 hover:border-slate-300 active:bg-slate-100 transition-colors duration-150">
                                    <ArrowRight size={14} strokeWidth={2.5} className="rotate-180" /><span>Regresar</span>
                                </button>
                                <button onClick={() => { if (validarArchivo()) setPaso(5) }} className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-medium text-white bg-blue-700 hover:bg-blue-800 active:bg-blue-900 active:scale-[0.99] shadow-sm transition-all duration-150">
                                    <span>Continuar</span><ArrowRight size={14} strokeWidth={2.5} />
                                </button>
                            </div>
                        </div>
                    ...
                    {/* Paso resumen (Sí: paso 4, No: paso 5) */}
                    {(paso === 4 && esTitularState === true) || (paso === 5 && esTitularState === false) ? (
                        <div className="space-y-4">
                            <div className="flex items-center gap-2">
                            ...
                            <div className="rounded-lg border border-slate-200 divide-y divide-slate-100">
                                ...
                                <div className="p-3 flex items-center justify-between">
                                    ...
                                    <span className="text-xs font-medium text-slate-900">
                                        {esTitularState === true
                                            ? [nombreInfractor, appaternoInfractor, apmaternoInfractor].filter(Boolean).join(' ') || 'Mismo que infractor'
                            ...
                            <div className="flex items-center gap-3">
                                <button onClick={() => setPaso(esTitularState === true ? 3 : 4)} className="inline-flex items-center justify-center gap-2 rounded-lg py-2.5 px-4 text-[13px] font-medium text-slate-600 bg-transparent border border-slate-200 hover:bg-slate-50 hover:border-slate-300 active:bg-slate-100 transition-colors duration-150">
                                    <ArrowRight size={14} strokeWidth={2.5} className="rotate-180" /><span>Regresar</span>
                                </button>

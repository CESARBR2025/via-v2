...
import { Shield, CheckCircle2, Loader2, FileText, User, Truck, CreditCard, Calendar, MapPin, Hash, X } from 'lucide-react'
...
export default function EntregarGarantiaButton({ detalle, onSuccess }: Props) {
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed"
        style={{
          background: pagoCompletado ? '#F0FDF4' : '#F1F5F9',
          color: pagoCompletado ? '#16A34A' : '#94A3B8',
          border: `1px solid ${pagoCompletado ? '#BBF7D0' : '#E2E8F0'}`,
        }}
      >
      ...
      {modalOpen && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: '#F1F5F9', background: '#F8FAFC' }}>
            ...
            <div className="px-6 py-5 space-y-5 max-h-[65vh] overflow-y-auto">
              ...
              <div className="rounded-xl p-4" style={{ background: '#F0FDF4', border: '1px solid #BBF7D0' }}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[11px] font-semibold tracking-[0.08em] uppercase text-[#16A34A] mb-1">Garantía retenida</p>
              ...
              {/* Monto */}
              <div className="rounded-xl p-4 flex items-center justify-between" style={{ background: '#FFF7ED', border: '1px solid #FED7AA' }}>
                <div>
                  <p className="text-[11px] font-semibold tracking-[0.08em] uppercase text-[#EA580C] mb-0.5">Monto de la infracción</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl p-4" style={{ background: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                  ...
                  <p className="text-[14px] font-semibold text-[#0F172A]">{detalle.datos_infractor.nombre_infractor || '—'}</p>
                  {detalle.datos_infractor.curp_infractor && (
                    <p className="text-[11px] text-[#64748B] font-mono mt-0.5">{detalle.datos_infractor.curp_infractor}</p>
                  )}
                </div>
                ...
                <div className="rounded-xl p-4" style={{ background: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                  ...
                  <p className="text-[11px] text-[#64748B] mt-0.5">{detalle.vehiculo.marca} {detalle.vehiculo.modelo} {detalle.vehiculo.anio}</p>
                </div>
              ...
              {detalle.ubicacion?.calle && (
                <div className="rounded-xl p-4" style={{ background: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                  ...
                  <p className="text-[13px] text-[#0F172A]">
                    {detalle.ubicacion.calle} {detalle.ubicacion.numero}, {detalle.ubicacion.cod_postal}, {detalle.ubicacion.municipio}, {detalle.ubicacion.estado}
                  </p>
                </div>
              )}
              ...
              <p className="text-[12px] text-[#64748B] leading-relaxed">
                Al confirmar, la garantía será liberada y la infracción pasará a estatus <strong style={{ color: '#0F172A' }}>Cerrada</strong>.
                Este proceso es irreversible.
              </p>
            </div>

            {/* Footer */}
            <div className="px-6 py-4 border-t flex items-center justify-between" style={{ borderColor: '#E2E8F0', background: '#FAFAFA' }}>
              <div className="flex items-center gap-2">
